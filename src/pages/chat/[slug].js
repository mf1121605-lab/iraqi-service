  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-b from-brand-950 via-brand-900 to-gold-900/40 text-white">
      ...
      <header className="relative z-10 flex items-center justify-between border-b border-white/10 bg-black/10 px-6 py-4 backdrop-blur">
        <Link href="/chat" className="text-sm text-white/70 underline underline-offset-4">
          {t('chat.backToRooms')}
        </Link>
        ...
        {settings?.chat_audio_track_key ? (
          <div>
            ...
            >
              {audioPlaying ? t('chat.audioPause') : t('chat.audioPlay')}
            </button>
          </div>
        ) : (
          <span />
      ...
      <main className="relative z-10 mx-auto flex h-[calc(100vh-136px)] max-w-3xl flex-col p-4">
        <div className="flex-1 space-y-3 overflow-y-auto">
          {messages.map((message) => (
            <div
              key={message.id}
              className={`max-w-[85%] rounded-xl2 px-4 py-2 ${
            ...
            >
              ...
              {canModerate(message) && (
                <button
                  ...
                  className="mt-1 text-xs text-white/50 underline"
                >
        ...
        <form onSubmit={handleSend} className="mt-3 flex items-center gap-2 rounded-xl2 bg-white/10 p-2">
          <input
            value={body}
          ...
          <button
            type="submit"
            className="rounded-xl2 bg-gold-500 px-4 py-2 text-sm font-semibold text-brand-950 hover:bg-gold-400"
          >
            {t('chat.sendCta')}
          </button>
        </form>
