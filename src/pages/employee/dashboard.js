      ...
      <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
        <aside className="space-y-6">
          <section className="rounded-2xl border border-black/5 bg-white/60 p-6 shadow-soft dark:border-white/10 dark:bg-surface-dark-alt/60">
            <div className="flex items-center gap-4">
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-brand-600 font-display text-lg font-bold text-white">
                {(profile.given_name ?? '?').charAt(0)}
              </div>
              <div>
                <p className="font-semibold">{fullName || '—'}</p>
                <p className="text-xs text-ink-muted dark:text-ink-dark-muted">{t('employeeDesk.profileTitle')}</p>
              </div>
            </div>
            <form onSubmit={saveSpecialization} className="mt-4 space-y-2">
              ...
              <input
                ...
                className="w-full rounded-xl2 border border-black/10 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400 dark:border-white/10 dark:bg-surface-dark"
              />
              <button
                type="submit"
                disabled={savingProfile}
                className="w-full rounded-xl2 bg-brand-600 px-3 py-2 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-50"
              >
                {t('common.save')}
              </button>
            </form>
        ...
        <section className="grid gap-6 lg:grid-cols-[320px_1fr]">
          <div className="rounded-2xl border border-black/5 bg-white/60 p-4 shadow-soft dark:border-white/10 dark:bg-surface-dark-alt/60">
            <h3 className="mb-3 font-semibold">{t('employeeDesk.queueTitle')}</h3>
            {queue === null ? (
                ...
                {queue.map((request) => (
                  <li key={request.id}>
                    <button
                      ...
                      className={`w-full rounded-xl2 border p-3 text-start text-sm transition ${
          ...
          <div className="rounded-2xl border border-black/5 bg-white/60 p-6 shadow-soft dark:border-white/10 dark:bg-surface-dark-alt/60">
            {!selectedRequest ? (
              ...
              <div className="space-y-6">
                ...
                <div>
                  <h4 className="text-sm font-semibold">{t('employeeDesk.statusHistoryTitle')}</h4>
                ...
                <div>
                  <h4 className="text-sm font-semibold">{t('employeeDesk.messagesTitle')}</h4>
