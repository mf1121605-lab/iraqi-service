          ...
          <div className="mb-2 flex items-center gap-1 border-b border-white/10 pb-2">
            {STICKER_PACKS.map((pack) => (
              <button
                ...
                className={`flex-1 truncate rounded-lg px-1.5 py-1 text-[11px] font-semibold transition-colors ${
          ...
          <div className="grid grid-cols-3 gap-1">
            {activePack.stickers.map((sticker) => (
              <button
                ...
                className="flex h-14 w-14 items-center justify-center rounded-xl text-3xl transition-colors hover:bg-white/10"
              >
                {sticker}
              </button>
            ))}
