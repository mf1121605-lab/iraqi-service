...
export default function AttachmentUploader({ pathPrefix, onUploaded, locale }) {
  ...
  return (
    <label className="flex cursor-pointer items-center text-sm font-semibold text-brand-700 hover:underline dark:text-brand-300">
      <input
        type="file"
        ...
        className="hidden"
      />
      {uploading ? t('chat.uploading') : t('chat.attachCta')}
